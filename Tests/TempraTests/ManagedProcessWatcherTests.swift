...
struct ManagedProcessWatcherTests {
    ...
    func defaultDebounceCoalescesProcessEvents() async {
        ...
        // Both events are delivered without awaiting in between, so the
        // debounced work item cannot run between them however loaded the
        // machine is; it needs this actor, which the test is holding.
